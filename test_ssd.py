...
def test_write(ssd, lba):
    ssd_nand = SSDNand()
    ssd_output = SSDOutput()
    ssd._write_ssd(lba=lba, value=TEST_WRITE_VALUE)
...
def test_erase_success(ssd):
    ssd_nand = SSDNand()
    for i in range(100):
        ssd._write_ssd(lba=i, value=TEST_WRITE_VALUE)
